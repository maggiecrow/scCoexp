sunset_plot_and_topology_analysis <- function (data, pheno, file_ext) {
...
for (i in 1:length(levels(as.factor(pheno$Batch)))){
		...
		#generate the coexpression network using Spearman correlation
		coexp=abs(cor(t(temp[,which(pheno$Batch==i)]),method="s"))
